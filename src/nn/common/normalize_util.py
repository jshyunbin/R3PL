from src.nn.common.pytorch_util import (
    dict_apply, 
    dict_apply_reduce, 
    dict_apply_split
)
from src.nn.modules import SingleFieldLinearNormalizer
import numpy as np
...
def get_resnet_imagenet_normalizer():
    """
    Returns a SingleFieldLinearNormalizer that applies ImageNet mean/std normalization,
    matching the preprocessing expected by torchvision pretrained ResNet models.

    Assumes input images are already in [0, 1] (e.g. after get_image_range_normalizer
    has been applied and then inverted, or directly from a [0,1]-scaled dataset).

    Forward transform: x_out = (x - mean) / std
    In scale/offset form (x * scale + offset):
        scale  = 1 / std
        offset = -mean / std
    """
